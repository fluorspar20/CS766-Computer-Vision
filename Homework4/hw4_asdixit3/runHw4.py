...
def challenge1b():
    ...
    # -------------------
    bg_pts = get_points_from_user(bg_img, 4, "Click corresponding 4 points in the background image")
    portrait_pts = get_points_from_user(portrait_img, 4, "Click 4 points in the portrait image")
...
def challenge1c():
    ...
    ransac_n = 1000  # TODO - Max number of iterations
    ransac_eps = 0.5  # TODO - Acceptable alignment error
...
def challenge1f():
    ...
    imgc = cv2.imread("custom_images/center.png").astype(np.float32) / 255.0
    imgl = cv2.imread("custom_images/left.png").astype(np.float32) / 255.0
    imgr = cv2.imread("custom_images/right.png").astype(np.float32) / 255.0

    stitched_img = stitch_img(imgl, imgc, imgr)
    cv2.imwrite("madison_panorama.png", (stitched_img * 255).astype(np.uint8))
