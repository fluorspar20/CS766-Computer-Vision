...
def stitch_img(*args):
    ...
    for n in range(num_imgs):
        ...
        Xs = np.array(kp_n)
        Xd = np.array(kp_stitched)

        inliers_id, H = run_ransac(Xs, Xd, ransac_n=1000, eps=3)
        H = compute_homography(Xs[inliers_id], Xd[inliers_id])

        H_inv = np.linalg.inv(H)
        mask_n, warped_n = backward_warp_img(img_n, H_inv, (W_stitched, H_stitched))

        warped_n = warped_n.astype(stitched_img.dtype)

        stitched_img = blend_image_pair(stitched_img, stitch_mask, warped_n, mask_n, mode="blend")
