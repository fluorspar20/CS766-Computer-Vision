...
def hw1_walkthrough2():
    ...
    # Normalize the grayscale image to be in the range [0, 1]
    gray_min = gray_iheartny_img.min()
    gray_max = gray_iheartny_img.max()
    gray_iheartny_img = (gray_iheartny_img - gray_min) / (gray_max - gray_min)

    # Convert the grayscale image into a binary mask using a threshold value
    threshold = 0.25  # reasonable default threshold in [0,1]
    ...
    resized_iheartny = np.pad(
        resized_iheartny, ((top_pad, bottom_pad), (0, 0),
                            (0, 0)), mode="constant")
    iresized_mask = np.pad(
        iresized_mask, ((top_pad, bottom_pad), (0, 0)),
        mode="constant")
    # plt.figure(), plt.imshow(iresized_mask, cmap="gray")
    # plt.title("Fully Padded Mask")

    resized_iheartny = resized_iheartny.astype(np.uint8)
    iresized_mask = iresized_mask.astype(bool)
